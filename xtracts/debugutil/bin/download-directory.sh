#!/bin/bash
# External tool for IntelliJ to download the contents of a directory via the
# Composum Nodes Debugutil Download Jcr Tree Servlet as a zip and unpack it,
...
# Relies on /bin/cpm/nodes/debug/downloadjcr.html${path}?packageName=$PKG
...
echo Arguments "$*"
echo Dir: $(pwd)
echo URL: $CPM_PROTOCOL://$CPM_HOST:$CPM_PORT/bin/cpm/nodes/debug/downloadjcr.html/$1
...
curl -s -S -o $TMPFIL -u $CPM_ADMINUSER:$CPM_ADMINPASSWD $CPM_PROTOCOL://$CPM_HOST:$CPM_PORT/bin/cpm/nodes/debug/downloadjcr.html/$1
...
if command -v 7z &> /dev/null
then
  7z x "$CPM_CPM_7Z_EXTRACT_EXCLUDE" -y $TMPFIL "jcr_root/$1/*"
