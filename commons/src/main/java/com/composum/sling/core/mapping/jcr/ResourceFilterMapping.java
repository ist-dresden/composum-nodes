...
public class ResourceFilterMapping {
    ...
    public static final Pattern STRING_PATTERN = Pattern.compile(
            "^(Name|Path|Type|PrimaryType|NodeType|ResourceType|MimeType|Property|All|Folder|ContentNode)\\((.*)\\)$"
    );
    /**
    ...
    public static final Pattern BALANCED_PARENTHESES = Pattern.compile(
            "[^()]*(?=\\()(?:(?=.*?\\((?!.*?\\1)(.*\\)(?!.*\\2).*))(?=.*?\\)(?!.*?\\2)(.*)).)+?.*?(?=\\1)[^(]*(?=\\2$)[^()]*");

    public static final String DEFAULT_FILTER_TYPE = "Path";

    /**
     * Deserializes a filter from its String representation generated by {@link #toString(ResourceFilter)}. If the rules
        ...
        if (matcher.matches()) {
            ...
            if (matcher.matches()) {
                ...
                try {
                    ...
                    if (ResourceFilter.FolderFilter.class.equals(filterClass)) {
                        filter = ResourceFilter.FOLDER;
                    } else if (!ResourceFilter.AllFilter.class.equals(filterClass)) {
                        try {
                            StringFilter stringFilter = StringFilterMapping.fromString(values);
                            if (stringFilter !=null) {
                                filter = (ResourceFilter) filterClass.getConstructor(StringFilter.class).newInstance(stringFilter);
                            } else {
                                filter = (ResourceFilter) filterClass.getConstructor(String.class).newInstance(values);
                            }
    ...
    static {
        ...
        STRATEGY_MAP.put(ResourceFilter.PropertyFilter.class, new PropertyFilterStrategy());
    ...
    public static class PatternFilterStrategy extends GeneralStrategy {
        ...
        protected ResourceFilter createInstance(ResourceHandle resource,
            ...
            if (stringFilterRes != null) {
                stringFilter = StringFilterMapping.fromResource(stringFilterRes);
            }
            return type.getConstructor(StringFilter.class).newInstance(stringFilter);
        }
    ...
    public static class PropertyFilterStrategy extends PatternFilterStrategy {

        @Override
        protected ResourceFilter createInstance(ResourceHandle resource,
                                                Class<? extends ResourceFilter> type)
                throws Exception {
            String propertyName = resource.getProperty(PROPERTY_PROP_NAME, "");
            Resource stringFilterRes = resource.getChild(NODE_NAME_FILTER);
            StringFilter stringFilter = null;
            if (stringFilterRes != null) {
                stringFilter = StringFilterMapping.fromResource(stringFilterRes);
            }
            return type.getConstructor(String.class, StringFilter.class).newInstance(propertyName, stringFilter);
        }

        @Override
        public void toResource(Resource resource, ResourceFilter filter) throws RepositoryException {
            ResourceHandle handle = ResourceHandle.use(resource);
            handle.setProperty(PROPERTY_PROP_NAME, ((ResourceFilter.PropertyFilter) filter).getPropertyName());
            super.toResource(resource, filter);
        }
    }
    ...
    public static class FilterSetStrategy extends GeneralStrategy {
        ...
        protected ResourceFilter createInstance(ResourceHandle resource,
            ...
            return type.getConstructor(ResourceFilter.FilterSet.Rule.class, List.class)
