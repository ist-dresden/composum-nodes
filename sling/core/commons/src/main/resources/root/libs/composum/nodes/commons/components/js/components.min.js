...
_.isFunction(this.view.validationReset)&&this.view.validationReset.apply(this.view)})},validate:function(a){var b=c["const"].form,e=!0;this.$(d["const"].css.selector.general).each(function(){this.view&&_.isFunction(this.view.isValid)&&(e=this.view.isValid.apply(this.view,[a])&&e)});e?(this.$el.removeClass(b.css.status.invalid),this.$el.addClass(b.css.status.valid)):(this.$el.removeClass(b.css.status.valid),this.$el.addClass(b.css.status.invalid));return e},getValues:function(){var a=c["const"].form,
...
this.$input.closest(".form-group").addClass("has-error")}return this.valid},reset:function(){this.valid=void 0;this.$input.closest(".form-group").removeClass("has-error");this.$input.text("")}});d.register(".widget.text-area-widget",c.TextAreaWidget);c.PathSelector=d.Widget.extend({initialize:function(a){d.Widget.prototype.initialize.apply(this,[a]);this.setPathWidget(a.pathWidget)},setPathWidget:function(a){this.pathWidget&&this.pathWidget.$input.off("change."+this.getEventId());if(this.pathWidget=
a)this.pathWidget.$input.on("change."+this.getEventId(),_.bind(this.pathInputChanged,this))},pathInputChanged:function(){if(!this.busy){this.busy=!0;var a=this.pathWidget.getValue();a!==this.lastPathSelected&&0===a.indexOf("/")&&e.getJson("/bin/cpm/nodes/node.tree.json"+a,_.bind(function(a){this.lastPathSelected=a.path;this.onPathChanged(a.path)},this));this.busy=!1}}});c.PathWidget=c.TextFieldWidget.extend({initialize:function(a){c.TextFieldWidget.prototype.initialize.apply(this,[a]);this.dialogTitle=
this.$el.attr("title");this.dialogLabel=this.$el.data("label");this.setRootPath(this.$el.data("root")||"/");this.filter=this.$el.data("filter");this.$textField.attr("autocomplete","off");this.$textField.typeahead({minLength:1,source:function(a,c){0===a.indexOf("/")&&e.getJson("/bin/cpm/nodes/node.typeahead.json"+a,function(a){c(a)})},matcher:function(a){var b=/^(.*\/)([^\/]*)$/.exec(this.query);return a.match(".*"+b[2]+".*")},highlighter:function(a){var b=/^(.*\/)([^\/]*)$/.exec(this.query);a=(new RegExp("^(.*)"+
b[2]+"(.*)?")).exec(a);return a[1]+"<b>"+b[2]+"</b>"+(a[2]||"")}});this.$selectButton=this.$("button.select");if(0<this.$selectButton.length)this.$selectButton.on("click",_.bind(this.selectPath,this))},selectPath:function(a){var b=e.getView("#path-select-dialog",c.SelectPathDialog);b.setTitle(this.dialogTitle);b.setLabel(this.dialogLabel);b.setRootPath(this.rootPath);b.setFilter(this.filter);b.show(_.bind(function(){this.getPath(_.bind(b.setValue,b))},this),_.bind(function(){this.setPath(b.getValue())},
this))},getPath:function(a){var b=this.getValue();if(_.isFunction(a))a(b);else return b},setPath:function(a){this.setValue(a)},getRootPath:function(){return this.rootPath?this.rootPath:"/"},setRootPath:function(a){this.rootPath=a}});d.register(".widget.path-widget",c.PathWidget);c.ReferenceWidget=c.PathWidget.extend({initialize:function(a){c.PathWidget.prototype.initialize.apply(this,[a]);this.filter||(this.filter="referenceable")},getPath:function(a){this.path?a(this.path):this.retrievePath(_.bind(function(b){this.path=
...
(a.showUpload=!0));if(b=this.$el.data("type"))a.fileType=b;this.whatever=this.$textField.fileinput({showPreview:void 0===a.showPreview?!0:a.showPreview,showUpload:a.showUpload||!1,fileType:a.fileType||"any"});this.$widget=this.$el.closest(".file-input-new");this.$inputCaption=this.$widget.find(".kv-fileinput-caption")},setValue:function(a){},grabFocus:function(){this.$inputCaption.focus()},reset:function(){this.$textField.fileinput("clear")}});d.register(".widget.file-upload-widget",c.FileUploadWidget);
c.PropertyNameWidget=c.TextFieldWidget.extend({initialize:function(a){c.TextFieldWidget.prototype.initialize.apply(this,[a]);this.$textField.typeahead({minLength:1,source:"jcr:data jcr:description jcr:lastModified jcr:lastModifiedBy jcr:mixinTypes jcr:primaryType jcr:title sling:redirect sling:resourceType".split(" ")})}});d.register(".widget.property-name-widget",c.PropertyNameWidget);c.RepositoryNameWidget=c.TextFieldWidget.extend({nameChanged:function(a,b){"jcr:primaryType"==a?b.setWidgetType.apply(b,
["jcr-primaryType"]):"jcr:mixinTypes"==a&&b.setWidgetType.apply(b,["jcr-mixinTypes"])}});d.register(".widget.repository-name-widget",c.RepositoryNameWidget);c.PrimaryTypeWidget=c.TextFieldWidget.extend({initialize:function(a){c.TextFieldWidget.prototype.initialize.apply(this,[a]);this.$textField.attr("autocomplete","off");this.$textField.typeahead({minLength:1,source:function(a,c){e.ajaxGet("/bin/cpm/core/system.primaryTypes.json",{query:a},function(a){c(a)})}})}});d.register(".widget.primary-type-widget",
c.PrimaryTypeWidget);c.MixinTypeWidget=c.TextFieldWidget.extend({initialize:function(a){c.TextFieldWidget.prototype.initialize.apply(this,[a]);this.$textField.attr("autocomplete","off");this.$textField.typeahead({minLength:1,source:function(a,c){e.ajaxGet("/bin/cpm/core/system.mixinTypes.json",{query:a},function(a){c(a)})}})}});d.register(".widget.mixin-type-widget",c.MixinTypeWidget)})(e.components,window.widgets)})(window.core);
