...
types:h.treeTypes};a.dragAndDrop&&(this.dragAndDrop=_.extend(this.dragAndDrop||{},a.dragAndDrop));this.dragAndDrop&&(b.plugins=_.union(b.plugins,["dnd"]),b.dnd=this.dragAndDrop);a.contextmenu&&(this.contextmenu=_.extend(this.contextmenu||{},a.contextmenu));this.contextmenu&&(b.plugins=_.union(b.plugins,["contextmenu"]),b.contextmenu=this.contextmenu);this.$jstree=this.$el.jstree(b);this.jstree=this.$el.jstree(!0);if(_.isFunction(this.renameNode))this.$jstree.off("dblclick").on("dblclick",".jstree-anchor",
_.bind(this.editNodeName,this));this.delegateEvents({"loaded.jstree":this.initSelectedData,"redraw.jstree":this.onRedrawNode,"open_node.jstree":this.onOpenNode,"select_node.jstree":this.nodeSelected});$(document).on("path:selected."+this.nodeIdPrefix+"Tree",_.bind(this.onPathSelected,this)).on("path:inserted."+this.nodeIdPrefix+"Tree",_.bind(this.onPathInserted,this)).on("path:changed."+this.nodeIdPrefix+"Tree",_.bind(this.onPathChanged,this)).on("path:moved."+this.nodeIdPrefix+"Tree",_.bind(this.onPathMoved,
this)).on("path:deleted."+this.nodeIdPrefix+"Tree",_.bind(this.onPathDeleted,this))},initSelectedData:function(){this.initialSelect||(this.initialSelect=this.$el.attr("data-selected"));this.initialSelect&&(this.selectNode(this.initialSelect),this.initialSelect=void 0);this.undelegate("loaded.jstree")},onPathSelected:function(a,b){var c=this.getSelectedTreeNode();console.log(this.nodeIdPrefix+"tree.onPathSelected("+b+"):"+c);if(!(c&&c.original.path==b||this.busy)){this.busy=!0;try{this.selectNode(b,
_.bind(function(a){if(!this.getSelectedTreeNode()&&_.isFunction(this.onPathSelectedFailed))this.onPathSelectedFailed(a)},this))}finally{this.busy=!1}}},onPathInserted:function(a,b,c){a=this.nodeId(b);console.log(this.nodeIdPrefix+"tree.onPathInserted("+b+","+c+"):"+a);this.refreshNodeById(a)},onPathChanged:function(a,b){var c=this.nodeId(b);console.log(this.nodeIdPrefix+"tree.onPathChanged("+b+"):"+c);this.refreshNodeById(c)},onPathMoved:function(a,b,c){a=this.getTreeNodeByPath(b);console.log(this.nodeIdPrefix+
"tree.onPathMoved("+b+","+c+"):"+a);if(a){var d=(a=this.getSelectedTreeNode())&&a.original.path==b;a=e.getParentPath(b);b=e.getParentPath(c);a!=b&&(a=this.nodeId(a),this.refreshNodeById(a));b=this.nodeId(b);this.jstree.get_node(b,!1).state.loaded=!1;this.refreshNodeById(b,_.bind(function(){d&&this.selectNode(c)},this))}},onPathDeleted:function(a,b){var c=this.getTreeNodeByPath(b);console.log(this.nodeIdPrefix+"tree.onPathDeleted("+b+"):"+c);if(c){var d=this.getSelectedTreeNode(),g=this.findNearestOfDeletion(b),
f=void 0;d&&d.original.path==b&&(f=this.findNearestOfDeletion(b));this.refreshNodeById(this.getParentNodeId(c.id),_.bind(function(){if(f){var a=f.original.path;this.selectNode(a)}g&&(a=g.original.path,this.get$TreeNodeAnchor(a).focus())},this))}},findNearestOfDeletion:function(a){if(a=this.getTreeNode(a)){var b=this.jstree.get_next_dom(a,!0);if(!b||1>b.length)b=this.jstree.get_prev_dom(a,!0);if(!b||1>b.length)b=this.jstree.get_prev_dom(a);if(b&&0<b.length)return this.jstree.get_node(b[0].id)}},nodeIdPrefix:"TR_",
nodeId:function(a){!a||"string"===typeof a&&0==a.indexOf(this.nodeIdPrefix)||(_.isArray(a)&&(a=a.join("/")),a=$.base64.encode(a).replace(/=/g,"-").replace(/[/]/g,"_"),a=this.nodeIdPrefix+a);return a},nodeData:function(a,b){var c=this.dataUrlForNode(a),d=this;console.log(this.nodeIdPrefix+"tree.nodeData("+a+"): "+c);e.getJson(c,function(a){a.id=d.nodeId(a.path);if(a.children)for(var c=0;c<a.children.length;c++)a.children[c].id=d.nodeId(a.children[c].path);b.call(d.$jstree,a)})},dataUrlForNode:function(a){var b;
a.original&&a.original.path&&(b=a.original.path);b=b?window.core.encodePath(b):"/";return _.isFunction(this.dataUrlForPath)?this.dataUrlForPath(b):"/bin/cpm/nodes/node.tree.json"+b+(this.filter&&"default"!=this.filter?"?filter="+this.filter:"")},refreshNodeStateById:function(a){var b=this.jstree.get_node(a);b&&b.original&&(a=this.jstree.get_node(a,!0),0<a.length&&this.refreshNodeState(a,b));return b},refreshNodeState:function(a,b){b.original.type&&a.attr("data-type",b.original.type);b.original.contentType&&
a.attr("data-content-type",b.original.contentType);b.original.treeType&&a.addClass(b.original.treeType);if(b.original.jcrState){var c=b.original.jcrState;c.checkedOut&&c.isVersionable&&a.addClass("checked-out");c.locked&&(a.addClass("locked"),c.lock.isDeep&&a.addClass("deep-lock"),c.lock.isHolder&&a.addClass("lock-holder"))}return b},getTreeNodeByPath:function(a){a=this.nodeId(a);return this.getTreeNodeById(a)},getTreeNodeById:function(a){return this.jstree.get_node(a)},editNodeName:function(a){a&&
a.preventDefault();a=this.jstree.get_selected();if(0<a.length){a=this.jstree.get_node(a[0]);var b=a.text;this.jstree.edit(a,null,_.bind(function(a,d,e){d&&!e&&(d=a.text.replace(/^\s+/,"").replace(/\s+$/,""),d!=b&&this.renameNode(a.original,b,d))},this))}},checkCallback:function(a,b,c,d,g){return"move_node"==a?(b&&c&&(a=c.original,c=b.original,c.path!=a.path&&((g=a.path==e.getParentPath(c.path))&&d==this.getNodeIndex(b)||(_.isFunction(this.dropNode)?this.dropNode(c,a,d,g):e.alert("warning","Drag Stop... (no implemented action)",
"dragged: "+c.path+"<br/>to target: "+a.path+" / pos: "+d+" ("+(g?"reorder":"move")+")")))),!1):!0},nodeSelected:function(a,b){var c=b.node.id,d=b.node.original.path,c=this.$("#"+(c?c:this.nodeId(d)));this.jstree.open_node(c);this.onNodeSelected(d,b.node,c)},onNodeSelected:function(a,b,c){this.busy||(console.log(this.nodeIdPrefix+"tree.onNodeSelected("+a+","+b+","+c+")"),$(document).trigger("path:select",[a,b,c]))},onRedrawNode:function(a,b){for(var c=0;c<b.nodes.length;c++)this.refreshNodeStateById(b.nodes[c])},
onOpenNode:function(a,b){if(b.node&&(this.refreshNodeStateById(b.node.id),b.node.children))for(var c=0;c<b.node.children.length;c++)this.refreshNodeStateById(b.node.children[c])},refreshNodeById:function(a,b){console.log(this.nodeIdPrefix+"tree.refreshNodeById("+a+")");if(!a){var c=this.jstree.get_selected();c&&0<c.length&&(a=c[0])}a&&(c=this.jstree.get_selected(),this.jstree.load_node(a,_.bind(function(){this.jstree.open_node(a,_.bind(function(){c&&this.jstree.get_node(c)&&(this.jstree.select_node(c,
!0),this.refreshNodeStateById(c));_.isFunction(b)&&b.call(this,c)},this))},this)))},getParentNodeId:function(a){var b=void 0;a&&(a=this.jstree.get_node(a))&&a.original&&(b=a.original.path,b=e.getParentPath(b),b=this.nodeId(b));return b},getNodeIndex:function(a){if(a&&(a=this.jstree.get_node(a))&&a.original){a=a.original.path;var b=e.getParentPath(a);if(b&&(b=this.nodeId(b),(b=this.jstree.get_node(b))&&b.original&&(b=b.original.children)))for(var c=0;c<b.length;c++)if(b[c].path==a)return c}}})})(e.components)})(window.core);
