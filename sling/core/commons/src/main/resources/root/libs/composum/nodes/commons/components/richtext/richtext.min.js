...
$(this),c=$(document.createElement("div")),d=a.find(".composum-widgets-richtext_value").clone();c.append(d);a.html(c.html())}});c.trumbowyg={openModalWidgets:function(b,c,d){var f=this;a.getHtml(b,function(b){b=f.openModal("Content Link",b);var e=a.getWidget(b[0],"form",a.components.FormWidget),h={};b.addClass("composum-widgets-richtext_link-dialog");window.widgets.setUp(e.el);e.$("[name]").each(function(){var b=$(this).attr("name");if(b){var d=a.widgetOf(this);if(d){var f=c[b];d.setValue(f?f:void 0);
h[b]=d}}});b.on("tbwconfirm",function(){if(e.validate()){for(var a in h)c[a]=h[a].getValue();d(c)&&(f.syncCode(),f.$c.trigger("tbwchange"),f.closeModal(),$(this).off("tbwconfirm"))}});b.one("tbwcancel",function(){$(this).off("tbwconfirm");f.closeModal()})})}};$.extend(!0,$.trumbowyg,{plugins:{contentLink:{init:function(a){a.addBtnDef("link",{fn:function(){var b=function(b){a.restoreRange();var c=$(['<a href="',b.url,'">',b.text,"</a>"].join(""));0<b.title.length&&c.attr("title",b.title);0<b.target.length&&
c.attr("target",b.target);a.range.deleteContents();a.range.insertNode(c[0]);return!0};(function(){for(var d=a.doc.getSelection(),f=d.focusNode,e={};0>["A","DIV"].indexOf(f.nodeName);)f=f.parentNode;if(f&&"A"===f.nodeName){var g=$(f);e.url=g.attr("href");e.title=g.attr("title");e.target=g.attr("target");g=a.doc.createRange();g.selectNode(f);d.addRange(g)}a.saveRange();e.text=a.getRangeText();c.trumbowyg.openModalWidgets.apply(a,["/libs/composum/nodes/commons/components/richtext/link/dialog.html",e,
b])})()}})}}}})})(a.components)})(window.core);
